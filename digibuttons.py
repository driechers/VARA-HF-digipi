...
def button_callback_24(number):
    ...
    if number == 24:                                       # start varanny
        try:
            cmd = "sudo systemctl is-active varanny"
            status = "active"
            cmd_output = subprocess.check_output(cmd, shell=True).decode("utf-8")
        except:
            status = "inactive"
        if status == "inactive":
            cmd = "sudo systemctl start varanny"
            cmd_output = subprocess.check_output(cmd, shell=True).decode("utf-8")
            ...
            if ( len(IP) < 5 ):
                IP = "0.0.0.0"
            subprocess.call(['/home/pi/digibanner.py', '-b', 'VARA HF', '-s', IP, '-d', getenv('NEWDISPLAYTYPE')])
        else:
            cmd = "hostname -I | cut -d' ' -f1"
            IP = subprocess.check_output(cmd, shell=True).decode("utf-8")
            if ( len(IP) < 5 ):
                IP = "0.0.0.0"
            cmd = "sudo systemctl stop varanny"
            cmd_output = subprocess.check_output(cmd, shell=True).decode("utf-8")
